...
if __name__ == '__main__':
    ...
    for class22 in class22Values:
        print("\nTECHNICAL 22 = ", class22, (df.technical_22 == class22).sum(), "\n")
    ...
    while True:
        ...
        for class22 in class22Values:
            dfp = pd.DataFrame(index=observation.features.id).loc(observation.features.technical_22 == class22)
            ...
            for factor in usedFactors:
                ...
                for class22 in class22Values:
                    for i, fr in assetClasses[int(class22 * 2 + 1)].factorRange.loc[factor].iterrows():
                        new[(raw >= fr["min"]) & (raw < fr["max"]) & (
                        observation.features.technical_22 == class22)] = fr.z
                    dfp[factor] = new
            dfp.fillna(0, inplace=True)

            # Create Market
            dfm = pd.DataFrame(dfp.loc[raw.technical_22 == class22].mean()).transpose()

            ac = assetClasses[int(class22 * 2 + 1)]

            # Create y predictions
            market = ac.elnetModel_Market.predict(dfm.loc[dfm.technical_22 == class22, usedFactors])
            ...
            target.loc[dfp.technical_22 == class22, 'y'] = dispersion * relVolatility * relAlpha
